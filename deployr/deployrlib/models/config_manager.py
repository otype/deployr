...
class ConfigManager(object):
    ...
    def load_config_from_file(self, config_file=None):
        ...
        logger.info("Setting configuration file {}".format(self.config_file))
        if not os.path.exists(self.config_file):
            logger.error("Cannot load configuration file {}: not found!".format(self.config_file))
            sys.exit(1)

        try:
            self.config = ConfigObj(self.config_file)
        except Exception, e:
            logger.error("Error on reading configuration file {}! Error: {}".format(self.config_file, e))
            sys.exit(1)
    ...
    def load_config(self, config_obj=None):
        ...
        try:
            ...
            logger.error("Error on reading configuration object {}! Error: {}".format(config_obj, e))
            sys.exit(1)
    ...
    def write_config(self):
        ...
        """
        logger.info("Writing configuration file {}".format(self.config_file))
        try:
            self.config.filename = self.config_file
            self.config.write()
        except Exception, e:
            logger.error("Error when writing configuration file {}! Error: {}".format(self.config_file, e))
            sys.exit(1)
    ...
    def setup_config_dir(self):
        ...
        try:
            ...
            logger.error("Error when creating configuration directory {}! Error: {}".format(fpath, e))
            sys.exit(1)
